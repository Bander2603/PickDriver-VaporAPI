...
private extension Database {
    var sql: any SQLDatabase {
        guard let sql = self as? (any SQLDatabase) else {
            fatalError("This migration requires an SQLDatabase (Postgres).")
        }
        return sql
    }
}

private func run(_ database: any Database, _ sql: String) -> EventLoopFuture<Void> {
    database.sql.raw(SQLQueryString(sql)).run()
}

// MARK: - Extensions (pg_trgm)

struct CreatePgTrgmExtension: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"CREATE EXTENSION IF NOT EXISTS pg_trgm;"#)
    }
...
struct CreateSeasons: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.seasons;"#)
    }
}
...
struct CreateUsers: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        ...
        -- Trigram
        CREATE INDEX trgm_idx_username ON public.users USING gin (username public.gin_trgm_ops);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.users;"#)
    }
}

// MARK: - f1_teams

struct CreateF1Teams: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.f1_teams;"#)
    }
}

// MARK: - drivers

struct CreateDrivers: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        -- Trigram
        CREATE INDEX trgm_idx_driver_name ON public.drivers USING gin (last_name public.gin_trgm_ops);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.drivers;"#)
    }
}

// MARK: - races

struct CreateRaces: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_races_completed_round ON public.races USING btree (completed, round);
        CREATE INDEX idx_races_season_completed ON public.races USING btree (season_id, completed);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.races;"#)
    }
}

// MARK: - leagues

struct CreateLeagues: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_leagues_season_id ON public.leagues USING btree (season_id);
        CREATE INDEX idx_leagues_status ON public.leagues USING btree (status);

        -- Trigram
        CREATE INDEX trgm_idx_league_name ON public.leagues USING gin (name public.gin_trgm_ops);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.leagues;"#)
    }
}

// MARK: - league_members

struct CreateLeagueMembers: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_league_members_league_user ON public.league_members USING btree (league_id, user_id);
        CREATE INDEX idx_league_members_user_id ON public.league_members USING btree (user_id);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.league_members;"#)
    }
}

// MARK: - league_teams

struct CreateLeagueTeams: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.league_teams;"#)
    }
}

// MARK: - team_members

struct CreateTeamMembers: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.team_members;"#)
    }
}

// MARK: - race_drafts

struct CreateRaceDrafts: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_race_drafts_league_status ON public.race_drafts USING btree (league_id, status);
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.race_drafts;"#)
    }
}

// MARK: - player_picks

struct CreatePlayerPicks: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_player_picks_banned ON public.player_picks USING btree (is_banned);
        CREATE INDEX idx_player_picks_draft_id ON public.player_picks USING btree (draft_id);
        CREATE INDEX idx_player_picks_driver ON public.player_picks USING btree (driver_id);

        -- Unique partial index: only one valid (non-banned) pick per (draft, user, mirror flag)
        CREATE UNIQUE INDEX unique_valid_pick
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.player_picks;"#)
    }
}

// MARK: - player_bans

struct CreatePlayerBans: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE UNIQUE INDEX idx_player_bans_solo
        ON public.player_bans USING btree (draft_id, user_id)
        WHERE (is_team_scope = false);

        CREATE UNIQUE INDEX idx_player_bans_team
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.player_bans;"#)
    }
}

// MARK: - race_results

struct CreateRaceResults: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
        ...
        CREATE INDEX idx_race_results_points ON public.race_results USING btree (race_id, points DESC);
        CREATE INDEX idx_race_results_position ON public.race_results USING btree (race_id, "position");
        """#)
    }

    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.race_results;"#)
    }
}

// MARK: - maintenance_stats

struct CreateMaintenanceStats: Migration {
    func prepare(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"""
    ...
    func revert(on database: any Database) -> EventLoopFuture<Void> {
        run(database, #"DROP TABLE IF EXISTS public.maintenance_stats;"#)
    }
}
