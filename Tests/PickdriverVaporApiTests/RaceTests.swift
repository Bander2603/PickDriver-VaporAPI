...
final class RaceTests: XCTestCase {
    func testGetAllRacesSortedByRound() async throws {
        ...
        // Insert out of order
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 2, name: "Race 2", completed: false, raceTime: Date().addingTimeInterval(6000))
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 1, name: "Race 1", completed: false, raceTime: Date().addingTimeInterval(5000))
    ...
    func testGetUpcomingRacesOnlyFutureSortedByRaceTime() async throws {
        ...
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 1, name: "Past", completed: true, raceTime: Date().addingTimeInterval(-3600))

        let t1 = Date().addingTimeInterval(7200)
        let t2 = Date().addingTimeInterval(10800)

        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 2, name: "Future 2", completed: false, raceTime: t2)
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 3, name: "Future 1", completed: false, raceTime: t1)
    ...
    func testGetCurrentReturnsClosestUpcomingNotCompletedRace() async throws {
        ...
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 1, name: "Far", completed: false, raceTime: far)
        let expected = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 2, name: "Near", completed: false, raceTime: near)
    ...
    func testGetCurrentReturns404WhenNoUpcomingRace() async throws {
        ...
        _ = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 1, name: "Completed Past", completed: true, raceTime: Date().addingTimeInterval(-3600))
    ...
    func testGetRaceByID() async throws {
        ...
        let season = try await TestSeed.createSeason(app: app)
        let race = try await TestSeed.createRace(app: app, seasonID: season.id!, round: 1, name: "Test GP", completed: false, raceTime: Date().addingTimeInterval(3600))
