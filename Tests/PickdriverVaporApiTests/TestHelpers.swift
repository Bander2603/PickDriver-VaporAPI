...
enum TestAuth {
    ...
    static func register(app: Application,
                         username: String? = nil,
                         email: String? = nil,
                         password: String = "123456") async throws -> CreatedUser {
...
enum TestSeed {
    ...
    static func createSeason(app: Application,
                             year: Int = 2026,
                             name: String = "Season \(Int.random(in: 2000...9999))",
                             active: Bool = true) async throws -> Season {
        let s = Season(year: year, name: name, active: active)
        try await s.save(on: app.db)
        return s
    }

    // MARK: - f1_teams (no model yet, so seed with SQL)
    ...
    static func createF1Team(app: Application,
                             seasonID: Int,
                             name: String = "Team \(Int.random(in: 1...9999))",
                             color: String = "#FFFFFF") async throws -> InsertedTeam {
        let sql = try XCTUnwrap(app.db as? (any SQLDatabase), "Database does not support SQL")
        ...
        let row = try await sql.raw(query)
    ...
    static func createRace(app: Application,
                           seasonID: Int,
                           round: Int,
                           name: String,
                           completed: Bool,
                           sprint: Bool = false,
                           fp1Time: Date? = nil,
                           raceTime: Date? = nil) async throws -> Race {
        let r = Race()
        r.seasonID = seasonID
        r.round = round
        r.name = name
    ...
    static func createDriver(app: Application,
                             seasonID: Int,
                             f1TeamID: Int,
                             firstName: String = "Test",
                             lastName: String = "Driver",
                             driverNumber: Int = Int.random(in: 2...99),
                             driverCode: String = "TST") async throws -> Driver {
        let d = Driver(
            seasonID: seasonID,
            teamID: f1TeamID,
            firstName: firstName,
